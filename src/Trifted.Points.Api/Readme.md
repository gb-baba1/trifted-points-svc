w# ASP.NET Core Web API Serverless Application
...
WORKDIR /src
COPY ["Trifted.Points.Api.csproj", "Trifted.Points.Api/"]
RUN dotnet restore "Trifted.Points.Api/Trifted.Points.Api.csproj"

WORKDIR "/src/Trifted.Points.Api"
COPY . .
RUN dotnet build "Trifted.Points.Api.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "Trifted.Points.Api.csproj" \
...
```
    cd "Trifted.Points.Api/test/Trifted.Points.Api.Tests"
    dotnet test
```

Deploy application
```
    cd "Trifted.Points.Api"
...
```
    cd "Trifted.Points.Api"
    dotnet-lambda deploy-serverless -cfg aws-lambda-tools.stage.json 
```
